...
def vali(model, vali_data, vali_loader, criterion,criterion_classification, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
            ...
            #print(classification_output.cpu().numpy())
            #print(batch_y_cls.cpu().numpy())
    ...
    if args.data == 'ttf':
        predictions = torch.from_numpy(np.concatenate(per_batch['predictions'], axis=0))
        threshold = 0.32
        predictions = (predictions > threshold).int().cpu().numpy().squeeze()
...
def test(model, test_data, test_loader, criterion, criterion_classification, args, device, itr):
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            preds.append(regression_output.unsqueeze(-1).cpu().numpy())
            trues.append(batch_y_reg.unsqueeze(-1).cpu().numpy())
