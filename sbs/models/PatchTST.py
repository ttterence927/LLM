...
class PatchTST(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.tmp = 24 #1312
        self.ln_proj_classification = nn.LayerNorm( self.tmp)
        self.ln_proj_forecasting = nn.LayerNorm( self.tmp)

        # Custom Activation Functions
        self.act_classification = F.gelu #nn.Linear(configs.d_ff , 2)
        self.act_forecasting = F.gelu #nn.Linear(configs.d_ff, 2)
        ...
        self.cnt = 0
        self.classification_head = nn.Linear(self.tmp, 1)
        self.forecast_head = nn.Linear(self.tmp, configs.pred_len)
    ...
    def forward(self, x, itr):
        ...
        outputs = self.enc_embedding(x, None)
        ...
        #print(classification_activations.shape)
        classification_activations = self.ln_proj_classification(classification_activations).reshape(B, -1)
