...
class PatchTST(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.ln_proj_classification = nn.LayerNorm(configs.d_model * (self.patch_num + 1) * configs.feature_num)
        self.ln_proj_forecasting = nn.LayerNorm(configs.d_model * (self.patch_num + 1)* configs.feature_num)
        ...
        self.cnt = 0
        self.classification_head = nn.Linear(configs.d_model * (self.patch_num+1)* configs.feature_num, self.num_classes)
        self.forecast_head = nn.Linear(configs.d_model * (self.patch_num+1)* configs.feature_num, configs.pred_len)
        self.padding_patch_layer = nn.ReplicationPad1d((0, self.stride)) 
        self.enc_embedding2 = DataEmbedding(self.patch_size, configs.d_model, configs.dropout)
        self.in_layer = nn.Linear(configs.patch_size, configs.d_model)
        self.out_layer = nn.Linear(configs.d_model * (self.patch_num +1), configs.pred_len)
    def forward(self, x, itr):
        ...
        x = self.in_layer(x)
        #x = self.enc_embedding2(x, None)
        #x, attns = self.encoder(x, attn_mask=None)
