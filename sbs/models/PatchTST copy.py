...
class PatchTST(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.act = F.gelu
        self.ln_proj = nn.LayerNorm(configs.d_model * (self.patch_num+1))
        ...
        self.enc_embedding2 = DataEmbedding(configs.feature_num * self.patch_size, configs.d_model, configs.dropout)
    def forward(self, x_enc, itr):
        ...
        classification_output = torch.sigmoid(self.classification_head(outputs))
        forecast_output = self.forecast_head(outputs)
        #print(classification_output.shape)
        if self.output_attention:
            return forecast_output, classification_output, 0 #attns
        else:
