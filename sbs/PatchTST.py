...
class PatchTST(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.enc_embedding2 = DataEmbedding(self.patch_size, configs.d_model, configs.dropout)
        self.in_layer = nn.Linear(configs.patch_size *2 + 3, configs.d_model)
    ...
    def embed_word_to_tensor(self, word, target_shape):
        ...
        # Adjust the size of the embedding to match the target shape
        target_embedding_size = 1 #target_shape[-1]
        if word_embedding.shape[0] > target_embedding_size:
            # Truncate the embedding if it's longer than the target size
            word_embedding = word_embedding[:target_embedding_size]
        elif word_embedding.shape[0] < target_embedding_size:
            # Pad the embedding if it's shorter than the target size
            padding = torch.zeros(target_embedding_size - word_embedding.shape[0])
            word_embedding = torch.cat([word_embedding, padding])

        # Expand and repeat the embedding to match the target shape
        expanded_embedding = word_embedding.unsqueeze(0).unsqueeze(0)
        expanded_embedding = expanded_embedding.repeat(target_shape[0], target_shape[1], 1)
    ...
    def forward(self, x, itr):
        ...
        #print(seasonal_word)
        #print(trend_word)
        out = torch.cat([seasonal_word, x, trend_word, x_2, prompt], axis=2)
