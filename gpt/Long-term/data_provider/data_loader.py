...
from utils.tools import convert_tsf_to_dataframe, Tokenizer, Config
import warnings
...
class Dataset_TTF(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 target='OT',class_target='call_12', scale=True, timeenc=0, freq='h', 
    ...
    def __read_data__(self):
        ...
        if self.scale:
            ...
            y_train_data = df_raw[[self.target]][border1s[0]:border2s[0]]
            self.scaler.fit(train_data.values)
            self.y_scaler.fit(y_train_data.values)
            # print(self.scaler.mean_)
            # exit()
            data = self.scaler.transform(df_data.values)
            y_data = self.y_scaler.transform(df_raw[[self.target]].values)
        else:
            data = df_data.values
            y_data = df_raw[[self.target]].values
        ...
        self.data_y = y_data[border1:border2] #scaling
        self.data_y_class = df_raw[[self.class_target]][border1:border2].values
    ...
    def __getitem__(self, index):
        ...
        seq_y_class = self.data_y_class[s_end:s_end+1]
        #print(seq_y_class)
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
        #print(seq_y.shape)
        #print(seq_y_class.shape)
        combined_seq = np.concatenate([seq_y, seq_y_class], axis=0)
        ...
        #print(s_begin,s_end)
        #print(r_begin,r_end)
        return seq_x, seq_y, seq_x_mark, seq_y_class
...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 percent=100, max_len=-1, train_all=False, num_classes=4):
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
    ...
    def __read_data__(self):
        ...
        config = Config(bin_size=1 / self.num_classes, max_coverage=0.9)  # Example configuration
        tokenizer = Tokenizer(config)
        encoded, params = tokenizer.encode(df_raw[[self.target]].values)
        self.data_y_class = encoded
        self.data_stamp = data_stamp
    ...
    def __getitem__(self, index):
        ...
        seq_x_mark = self.data_stamp[s_begin:s_end]
        seq_y_mark = self.data_stamp[r_begin:r_end]
        seq_y_class = self.data_y_class[r_begin+1:r_begin+2]
        return seq_x, seq_y, seq_x_mark, seq_y_class
