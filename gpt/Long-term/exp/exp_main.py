...
class Exp_Main(Exp_Basic):
    ...
    def _select_criterion(self, loss_name='MSE'):
        if loss_name == 'MSE':
            ...
            from loss.dilate_loss import tildeq_loss
            print('using tildeq')
            alpha = 0.55
            gamma  = 0.02
    ...
    def vali(self, vali_data, vali_loader, criterion,is_test = True):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(vali_loader)):
                ...
                if (self.args.model == 'GPT4TS') and not is_test:
    ...
    def train(self, setting):
        ...
        c = self._select_criterion(self.args.loss)
        # c = nn.MSELoss()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            print("number of self.model params: {}".format(params))
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(train_loader)):
                ...
                if (i + 1) % 100 == 0:
                    # for i in range(self.args.gpt_layers):
                    #     print("layer_{}, T_gate = {:.4f}, C_gate = {:.4f}, F_gate = {:.4f}".format(i,
                    #             (1 / (1 + (-self.args.scale * self.model.gpt2.h[i].adapter_layers_gate_T).exp())).item(),
                    #             (1 / (1 + (-self.args.scale * self.model.gpt2.h[i].adapter_layers_gate_C).exp())).item(),
                    #             (1 / (1 + (-self.args.scale * self.model.gpt2.h[i].attn.adapter_layers_gate).exp())).item(),
                    #         ))
